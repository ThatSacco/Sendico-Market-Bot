...
class LotVisionAnalyzer:
    ...
    def __init__(
        ...
        *,
        ...
        self.service_tier = str(service_tier or "on_demand").strip() or "on_demand"
    ...
    def _post_model_request(
        ...
        *,
        ...
        selected_tier = self.service_tier if service_tier is None else service_tier
        if selected_tier:
            payload["service_tier"] = selected_tier
        if json_mode:
        ...
        for model in candidates:
            ...
            while True:
                ...
                if (
                    ...
                    and (
                        "service_tier" in lowered_message
                        or "service tier" in lowered_message
                    )
                ):
                    retry_tiers: list[str] = []
                    if self.service_tier != "on_demand":
                        retry_tiers.append("on_demand")
                    retry_tiers.append("")

                    for retry_tier in retry_tiers:
                        label = retry_tier or "Groq default (field omitted)"
                        LOGGER.warning(
                            "Groq model %s rejected service tier %s; retrying with %s",
                            model,
                            self.service_tier,
                            label,
                        )
