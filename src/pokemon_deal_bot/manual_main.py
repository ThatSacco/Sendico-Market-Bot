...
class ManualScanRequest:
    """A bounded scan for one selected watchlist entry.

    Card details and Sendico search terms come from ``data/watchlist.yaml``.
    GitHub Actions supplies only the watchlist id, a PriceCharting product URL,
    and conservative run limits.
    """
    ...
    @classmethod
    def from_environment(cls, config_path: str | Path = "config.yaml") -> "ManualScanRequest":
        watchlist_id = _required_env("SENDICO_WATCHLIST_ID")
        target = select_watchlist_target(config_path, watchlist_id)
        search_terms = tuple(parse_search_terms(target.era_lot_search_terms))
    ...
    def validated(self) -> "ManualScanRequest":
        if self.target.match_mode != "exact_card":
            raise ValueError(
                "Manual PriceCharting scans currently require an exact_card "
                f"watchlist entry; {self.target.id!r} uses {self.target.match_mode!r}"
            )
        if not str(self.target.card_number or "").strip():
            raise ValueError(
                f"Watchlist entry {self.target.id!r} requires a printed card number"
            )
        validate_pricecharting_url(self.pricecharting_url)
        if not self.search_terms:
            raise ValueError(
                f"Watchlist entry {self.target.id!r} has no era_lot_search_terms. "
                "Add 1-4 focused Sendico terms in data/watchlist.yaml."
            )
        if len(self.search_terms) > _MAX_SEARCH_TERMS:
            raise ValueError(
                f"Watchlist entry {self.target.id!r} has {len(self.search_terms)} "
                f"era_lot_search_terms; use no more than {_MAX_SEARCH_TERMS} per run"
            )
    ...
    def target_name(self) -> str:
        if self.target.english_names:
            return self.target.english_names[0]
        if self.target.japanese_names:
            return self.target.japanese_names[0]
        return self.target.id

    @property
    def card_number(self) -> str:
        return str(self.target.card_number or "").strip()

    @property
    def set_name(self) -> str:
        return str(self.target.set_name or "").strip()

    @property
    def set_code(self) -> str:
        return str(self.target.set_code or "").strip()
...
def parse_search_terms(raw: str | Iterable[str]) -> list[str]:
    """Clean, deduplicate and validate watchlist search terms."""
...
def select_watchlist_target(
    config_path: str | Path,
    watchlist_id: str,
) -> WatchCard:
    config = load_config(config_path)
    targets = load_watchlist(config)
    wanted = str(watchlist_id or "").strip()
    for target in targets:
        if target.id == wanted:
            return target
    available = ", ".join(target.id for target in targets)
    raise ValueError(
        f"Active watchlist id {wanted!r} was not found. Available ids: {available}"
    )
...
def build_watch_card(request: ManualScanRequest) -> WatchCard:
    """Clone the selected watchlist entry with the run's PriceCharting URL."""
...
def build_runtime_config(
    ...
    vision["pipeline_state_version"] = "gemini-watchlist-bounded-search-v2"
...
async def _bounded_scroll_search_results(self: Any, page: Any) -> None:
    ...
    for round_number in range(maximum_rounds + 1):
        ...
        if current_count >= raw_link_limit:
            LOGGER.info("Stopping Sendico result loading at the configured raw-link cap")
            return
        ...
        "embeds": [
            {
                ...
                "description": (
                    f"Searching for **{request.target.display_name}** using the "
                    "focused terms stored in `data/watchlist.yaml`."
                ),
                "color": 0x5865F2,
                "fields": [
                    {
                        "name": "Watchlist entry",
                        "value": f"`{request.target.id}`",
                        "inline": False,
                    },
                    {
                        "name": "Search terms",
...
def request_summary(request: ManualScanRequest) -> str:
    return (
        f"Watchlist id: {request.target.id} | "
        f"Target: {request.target.display_name} | "
        f"Set: {request.set_name} {request.set_code} | "
...
async def run(
    ...
    *,
    ...
    request = ManualScanRequest.from_environment(config_path)
    LOGGER.info("Validated watchlist scan request: %s", request_summary(request))
...
def cli() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run a bounded Sendico scan for one watchlist id. Search terms are "
            "read only from that entry's era_lot_search_terms in data/watchlist.yaml."
        )
    )
    ...
    try:
        ...
        LOGGER.error("Invalid manual scan input or watchlist entry: %s", exc)
        exit_code = 2
